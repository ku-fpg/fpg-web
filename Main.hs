...
--        want ["_make/html/index.html"]
        ...
        (`elem` map fst build_cmds) ?> \ out -> case lookup out build_cmds of
                Just Copy        -> do
                ...
                Just Redirect    -> do
                        redirects <- readMeta "redirect.txt"
...
--                liftIO $ prepareDirectory out
        ...
        "_make/meta/contents.txt" *> \ out -> do
                alwaysRerun
                build_cmds <- liftIO $ findBuildDirections
                writeMeta "contents.txt" $ build_cmds

        "_make/meta/redirect.txt" *> \ out -> do
                alwaysRerun
                writeMeta "redirect.txt" $ redirects


        "_make/autogen/Sitemap.html" *> \ out -> do
                alwaysRerun
                writeFileChanged out $ "SITEMAP"
...
findBuildDirections :: IO [(String,Build)]
findBuildDirections = do
...
writeMeta nm = writeFileChanged ("_make/meta/" ++ nm) . show
