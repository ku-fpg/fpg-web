...
--                liftIO $ prepareDirectory out
        ...
        sequence [ urlRules $ htmlPage (dropDirectory1 $ dropDirectory1 $ file) "contents" idR
                 | (file,FromContent) <- bld
                 ]

        sequence [ urlRules $ htmlPage (dropDirectory1 $ dropDirectory1 $ file) "autogen" idR
                 | (file,AutoGenerated) <- bld
                 ]

        sequence [ urlRules $ copyPage (dropDirectory1 $ dropDirectory1 $ file)
                 | (file,Copy) <- bld
                 ]

        sequence [ urlRules $ redirectPage (dropDirectory1 $ dropDirectory1 $ file)
                 | (file,Redirect) <- bld
                 ]
