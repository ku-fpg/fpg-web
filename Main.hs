...
--    build_cmds <- findBuildDirections
    ...
    meta_redirect :: ShakeVar [(String,String)] <- newShakeVar "_make/meta/redirect.txt" $ do
            alwaysRerun     -- because this is part of the build
            return redirects

    meta_bibtex   :: ShakeVar [(String,BibTeX)] <- newShakeVar "_make/meta/bibtex.txt" $ do
            alwaysRerun     -- because we are reading the filesystem.
            need ["data/fpg.bib"]
            liftIO $ readBibTeX "data/fpg.bib"

    meta_contents :: ShakeVar [(String,Build)]  <- newShakeVar "_make/meta/contents.txt" $ do
            alwaysRerun     -- because we are reading the filesystem.
            bib <- readShakeVar meta_bibtex
            redir <- readShakeVar meta_redirect
    ...
    bld <- do
            ...
            redir <- return redirects
            bld <- findBuildTargets
            return $ bld ++
                        [ (html_dir </> file,Redirect)
                        | file <- map fst redir
                        ] ++
    ...
    let myURLs = [ htmlPage (dropDirectory1 $ dropDirectory1 $ file) "contents" $
                        wrapTemplateFile "template/page.html" (dropDirectory1 $ dropDirectory1 $ file)

                 | (file,FromContent) <- bld
                 ] ++
                 [ htmlPage (dropDirectory1 $ dropDirectory1 $ file) "autogen" idR
                 | (file,AutoGenerated) <- bld
                 ] ++
                 [ copyPage (dropDirectory1 $ dropDirectory1 $ file)
                 | (file,Copy) <- bld
                 ] ++
                 [ redirectPage (dropDirectory1 $ dropDirectory1 $ file)
                 | (file,Redirect) <- bld
                 ]
...
-}
        ...
        action $ do
                ...
                need $ filter (not . isAdminFile) $ map (\ (MyURL file _) -> html_dir </> file) $ myURLs
